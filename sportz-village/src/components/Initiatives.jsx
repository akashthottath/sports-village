      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        <div>
          ...
          <h2 className="mt-3 font-display font-extrabold text-3xl sm:text-4xl text-navy">
            <span className="underline-swipe">#SportForChange</span> In Action
          </h2>
        </div>
        <a href="#programs" className="btn-primary self-start sm:self-auto">
          Explore More
        </a>
      </div>
      ...
      <div className="mt-12 grid sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {initiatives.map((item) => (
          <article key={item.title} className="group">
            ...
            <h3 className="mt-4 font-display font-bold text-lg text-navy">{item.title}</h3>
